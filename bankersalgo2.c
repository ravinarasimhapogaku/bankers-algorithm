...
void displayTabularForm() {
    printf("\n\nAllocation\tMax Required\tNeed\n");
    printf("Process\tResources\tResources\tResources\n");
    printf("\t");
...
int main(int argc, char** argv) {
	srand(time(NULL));
        ...
        printf("\n");
        for(int i=0; i<nProcesses; i++) {
                printf("\nResource allocated to process P%d (R1 R2 ...)? ", i+1);
                for(int j=0; j<nResources; j++)
	...
	// maximum required resources
        for(int i=0; i<nProcesses; i++) {
                printf("\nMax resource for  process P%d (R1 R2 ...)? ", i+1);
                for(int j=0; j<nResources; j++)
	...
	safeSeq = (int *)malloc(nProcesses * sizeof(*safeSeq));
        ...
        if(!getSafeSeq()) {
                printf("\nUnsafe State! The processes leads the system to a deadlock.\n\n");
                exit(-1);
        }
